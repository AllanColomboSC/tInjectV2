...
type
  ...
  TFrmConsole = class(TForm)
    ...
    procedure Chromium1ConsoleMessage(Sender: TObject;
      const browser: ICefBrowser; level: Cardinal;
  ...
  protected
    ...
    procedure RequestCloseInject(var aMessage: TMessage);
      message FrmConsole_Browser_Direto;
  ...
  private
    ...
    procedure ExecuteJS(PScript: WideString; PDirect: Boolean = false;
      Purl: String = 'about:blank'; pStartline: Integer = 0);
    procedure ExecuteJSDir(PScript: WideString; Purl: String = 'about:blank';
      pStartline: Integer = 0);
    ...
    Procedure SendNotificationCenterDirect(PValor: TTypeHeader;
      Const PSender: TObject = nil);
  ...
  public
    ...
    Property MonitorLowBattry: Boolean Read FMonitorLowBattry
      Write FMonitorLowBattry;
    Property OnNotificationCenter: TNotificationCenter
      Read FOnNotificationCenter Write FOnNotificationCenter;
...
procedure TFrmConsole.ExecuteJS(PScript: WideString; PDirect: Boolean;
  Purl: String; pStartline: Integer);
var
  lThread: TThread;
begin
        ...
        TThread.Synchronize(nil,
          ...
          begin
            if Assigned(FrmConsole) then
              FrmConsole.Chromium1.browser.MainFrame.ExecuteJavaScript(PScript,
                Purl, pStartline)
          end);
      end);
    lThread.Start;
...
procedure TFrmConsole.ExecuteJSDir(PScript: WideString; Purl: String;
pStartline: Integer);
begin
  Chromium1.browser.MainFrame.ExecuteJavaScript(PScript, Purl, pStartline)
end;
...
begin
  ...
  if (TQR_Http in TQrCodeClass(pClass).Tags) or
    (TQR_Img in TQrCodeClass(pClass).Tags) then
  Begin
    FrmQRCode.Hide;
    Exit;
  End;
...
begin
  ...
  LJS := FrmConsole_JS_AlterVar(LJS, '<#PROFILE_PICTHUMB_URL#>',
    Trim(AProfilePicThumbURL));
  ExecuteJS(LJS, false);
end;
...
procedure TFrmConsole.StartQrCode(PQrCodeType: TFormQrCodeType;
PViewForm: Boolean);
begin
...
begin
  ...
  // AjustNameFile(vFileName) Alterado em 20/02/2020 by Lucas
...
procedure TFrmConsole.SendNotificationCenterDirect(PValor: TTypeHeader;
...
procedure TFrmConsole.EnviaBotao(Numero, Mensagem, Descricao: String;
const Botao, ID: array of String);
const
  tipoBtn = '1';
var
...
begin
  ...
  Mensagem := CaractersWeb(Mensagem);
  Descricao := CaractersWeb(Descricao);

  { FrmConsole_JS_VAR_SendButton =
    'WAPI.sendButtons("<#NUMERO#>", "<#CORPO#>, "<#BUTTON#>, "<#OPCAO#>);'; }

  for i := 0 to length(Botao) - 1 do
  begin
    if i = 0 then
    begin
      Btn := '{buttonId: ' + QuotedStr(ID[i]) + ', buttonText: {displayText: ' +
        QuotedStr(Botao[i]) + '}, type: ' + tipoBtn + '}';
    end
    else
      Btn := Btn + ',{buttonId: ' + QuotedStr(ID[i]) +
        ', buttonText: {displayText: ' + QuotedStr(Botao[i]) + '}, type: ' +
        tipoBtn + '}';
  end;

  Btn := '[' + Btn + ']';
  // {buttonId: 'id1', buttonText: {displayText: 'SIM'}, type: 1}

  LJS := FrmConsole_JS_VAR_SendTyping + FrmConsole_JS_VAR_SendButton;
  FrmConsole_JS_AlterVar(LJS, '<#NUMERO#>', QuotedStr(Trim(Numero)));
  FrmConsole_JS_AlterVar(LJS, '<#CORPO#>', QuotedStr(Trim(Mensagem)));
  FrmConsole_JS_AlterVar(LJS, '<#OPCAO#>', QuotedStr(Trim(Descricao)));
  FrmConsole_JS_AlterVar(LJS, '<#BUTTON#>', Trim(Btn));

  ExecuteJS(LJS, True);
end;

procedure TFrmConsole.ExecuteCommandConsole(const PResponse
...
begin
  ...
  // Nao veio nada
  if (PResponse.JsonString = '') or
    (PResponse.JsonString = FrmConsole_JS_RetornoVazio) Then
    Exit;
  ...
  Case PResponse.TypeHeader of
    ...
    Th_getQrCodeWEB, Th_getQrCodeForm:
      Begin
    ...
    Th_GetBatteryLevel:
      begin
        ...
        Begin
          ...
          FOnNotificationCenter(PResponse.TypeHeader,
            TResponseBattery(LOutClass).Result);
          FreeAndNil(LOutClass);
        End;
      end;
    ...
    Th_getMyNumber:
      Begin
        ...
        Begin
          ...
          FOnNotificationCenter(PResponse.TypeHeader,
            TResponseMyNumber(LOutClass).Result);
          FreeAndNil(LOutClass);
        End;
      End;

    Th_GetCheckIsValidNumber:
      begin
        If Assigned(FOnNotificationCenter) Then
        Begin
    ...
    Th_GetStatusMessage:
      begin
        LResultStr := copy(LResultStr, 11, length(LResultStr));
        // REMOVENDO RESULT
        LResultStr := copy(LResultStr, 0, length(LResultStr) - 1);
        // REMOVENDO }
...
begin
  ...
  Begin
    if (message = FrmConsole_JS_Ignorar) or
      (message = FrmConsole_JS_RetornoVazio) then
      Exit;
  End;

  LogAdd(message, 'CONSOLE');

  if message <> 'Uncaught (in promise) TypeError: output.update is not a function'
  then
...
begin
  ...
  try
    case Chromium1.ProxyScheme of
      psSOCKS4:
        lForm.ProxySchemeCb.ItemIndex := 1;
      psSOCKS5:
        lForm.ProxySchemeCb.ItemIndex := 2;
    else
      lForm.ProxySchemeCb.ItemIndex := 0;
    end;
    ...
    begin
      ...
      try
        ...
        GlobalCEFApp.IniFIle.WriteInteger('Config NetWork', 'ProxyType',
          Chromium1.ProxyType);
        GlobalCEFApp.IniFIle.WriteString('Config NetWork', 'ProxyServer',
          Chromium1.ProxyServer);
        GlobalCEFApp.IniFIle.WriteString('Config NetWork', 'ProxyPort',
          Chromium1.ProxyPort.ToString);
        GlobalCEFApp.IniFIle.WriteString('Config NetWork', 'ProxyUsername',
          Chromium1.ProxyUsername);
        GlobalCEFApp.IniFIle.WriteString('Config NetWork', 'ProxyPassword',
          Chromium1.ProxyPassword);
        GlobalCEFApp.IniFIle.WriteString('Config NetWork', 'ProxyScriptURL',
          Chromium1.ProxyScriptURL);
        GlobalCEFApp.IniFIle.WriteString('Config NetWork', 'ProxyByPassList',
          Chromium1.ProxyByPassList);
        GlobalCEFApp.IniFIle.WriteString('Config NetWork', 'CustomHeaderName',
          Chromium1.CustomHeaderName);
        GlobalCEFApp.IniFIle.WriteString('Config NetWork', 'CustomHeaderValue',
          Chromium1.CustomHeaderValue);
        GlobalCEFApp.IniFIle.WriteInteger('Config NetWork',
          'MaxConnectionsPerProxy', Chromium1.MaxConnectionsPerProxy);
        GlobalCEFApp.IniFIle.WriteInteger('Config NetWork', 'ProxyScheme',
          lForm.ProxySchemeCb.ItemIndex);
        ReloaderWeb;
      Except
        Result := false;
      end;
...
begin
  ...
  try
    ...
    if TInject(FOwner).CEF <> nil then
      FConectado := Chromium1.CreateBrowser(TInject(FOwner).CEF)
    else
...
procedure TFrmConsole.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  CanClose := FCanClose;
...
var
  LTempMessage: string;
begin
  LTempMessage := Text_System_memUse +
    IntToStr(GlobalCEFApp.UsedMemory div BYTES_PER_MEGABYTE) + ' Mb' + #13 +
    Text_System_memTot +
    IntToStr(GlobalCEFApp.TotalSystemMemory div BYTES_PER_MEGABYTE) + ' Mb' +
    #13 + Text_System_memFree +
    IntToStr(GlobalCEFApp.AvailableSystemMemory div BYTES_PER_MEGABYTE) + ' Mb'
    + #13 + Text_System_memFree +
    IntToStr(GlobalCEFApp.SystemMemoryLoad) + ' %';
