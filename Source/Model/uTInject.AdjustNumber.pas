...
type
  ...
  published
    ...
    property LengthPhone: Integer read FLengthPhone write FLengthPhone
      default 9;
    property AllowOneDigitMore: Boolean read FAllowOneDigit write FAllowOneDigit
      default True;
...
begin
  ...
  try
    ...
    if Length(LClearNum) <= (LengthDDI + LengthDDD + LengthPhone + 1 + LInc)
    Then // 14 then
    begin
      if (Length(LClearNum) <= (LengthDDD + LengthPhone + LInc)) or
        (Length(PNum) <= (LengthDDD + LengthPhone + LInc)) then
      begin
...
begin
  ...
  Begin
    if Length(Pnumero) = (LengthDDI + LengthDDD + LengthPhone +
      Length(CardContact)) then
    Begin
