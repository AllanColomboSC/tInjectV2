...
Const
  ...
  TInjectVersion = '3.2.0.0'; // 26/11/2020  //Alterado por Mike Lustosa
  CardContact = '@c.us';
  CardGroup = '@g.us';
  CardList = '@broadcast';
  ...
  JsonOptionClassPadrao = [joDateIsUTC, joDateFormatISO8601,
  ...
  FrmConsole_JS_Ignorar =
    '{"name":"getUnreadMessages","result":"{\"result\":[]}"}';
  ...
  FrmConsole_JS_WEBmonitorQRCode =
    'var AQrCode = document.getElementsByTagName("canvas")[0].toDataURL("image/png");console.log(JSON.stringify({"name":"getQrCodeWEB","result":{AQrCode}}));';
  ...
  FrmConsole_JS_monitorQRCode =
    'var AQrCode = document.getElementsByTagName("canvas")[0].toDataURL("image/png");console.log(JSON.stringify({"name":"getQrCode","result":{AQrCode}}));';
  ...
  FrmConsole_JS_VAR_DeleteMessages =
    'window.WAPI.deleteConversation("<#MSG_PHONE#>")';
  FrmConsole_JS_VAR_SendBase64 =
    'window.WAPI.sendImage("<#MSG_BASE64#>","<#MSG_PHONE#>", "<#MSG_NOMEARQUIVO#>", "<#MSG_CORPO#>")';
  FrmConsole_JS_VAR_SendMsg =
    'window.WAPI.sendMessageToID("<#MSG_PHONE#>","<#MSG_CORPO#>")';
  // FrmConsole_JS_VAR_SendTyping          = 'Store.WapQuery.sendChatstateComposing("<#MSG_PHONE#>");';
  FrmConsole_JS_VAR_SendTyping = '';
  FrmConsole_JS_VAR_SendLinkPreview =
    'window.WAPI.sendLinkWithAutoPreview("<#MSG_PHONE#>", "<#MSG_LINK#>", "<#MSG_CORPO#>")';
  FrmConsole_JS_VAR_SendLocation =
    'window.WAPI.sendLocation("<#MSG_PHONE#>", "<#MSG_LAT#>", "<#MSG_LNG#>", "<#MSG_CORPO#>")';
  ...
  FrmConsole_JS_VAR_SendContact =
    'window.WAPI.sendVCard("<#MSG_PHONE_DEST#>","BEGIN:VCARD\nVERSION:3.0\nN:;0;;;\nFN:<#MSG_NAMECONTACT#>\nTEL;type=CELL;waid=<#MSG_PHONE#>:<#MSG_PHONE#>\nEND:VCARD");';
  ...
  FrmConsole_JS_VAR_CheckIsValidNumber =
    'window.WAPI.isValidNumber("<#MSG_PHONE#>")' +
  ...
  FrmConsole_JS_VAR_ProfilePicThumb =
  ...
  FrmConsole_JS_VAR_getProfilePicThumb =
    'window.WAPI.teste("<#PROFILE_PICTHUMB_URL#>");';
  FrmConsole_JS_VAR_CreateGroup =
    'window.WAPI.createGroup("<#GROUP_NAME#>", "<#PARTICIPANT_NUMBER#>");setTimeout(function(){ window.WAPI.getAllGroups(); }, 3000);';

  FrmConsole_JS_GetAllGroups = 'window.WAPI.getAllGroups();';
  // 'window.WAPI.listMyGroups();';
  FrmConsole_JS_GetGroupAdmins = 'window.WAPI.getGroupAdmins("<#GROUP_ID#>");';
  FrmConsole_JS_VAR_listGroupContacts =
    'window.WAPI.getGroupParticipantIDs("<#GROUP_ID#>");';
  FrmConsole_JS_VAR_groupAddParticipant =
  ...
  FrmConsole_JS_VAR_groupRemoveParticipant =
  ...
  FrmConsole_JS_VAR_groupPromoteParticipant =
    'window.WAPI.promoteParticipant("<#GROUP_ID#>", "<#PARTICIPANT_NUMBER#>");setTimeout(function(){ window.WAPI.getGroupAdmins("<#GROUP_ID#>"); }, 3000);';
  FrmConsole_JS_VAR_groupDemoteParticipant =
    'window.WAPI.demoteParticipant("<#GROUP_ID#>", "<#PARTICIPANT_NUMBER#>");setTimeout(function(){ window.WAPI.getGroupAdmins("<#GROUP_ID#>"); }, 3000);';
  FrmConsole_JS_VAR_groupLeave = 'window.WAPI.leaveGroup("<#GROUP_ID#>");';
  FrmConsole_JS_VAR_groupDelete =
    'window.WAPI.deleteConversation("<#GROUP_ID#>");setTimeout(function(){ window.WAPI.getAllGroups(); }, 3000);';
  FrmConsole_JS_VAR_groupJoinViaLink =
    'window.WAPI.joinGroupViaLink("<#GROUP_LINK#>");setTimeout(function(){ window.WAPI.getAllGroups(); }, 3000);';
  ...
  FrmConsole_JS_VAR_getGroupInviteLink =
    'window.WAPI.getGroupInviteLink("<#GROUP_ID#>");';
  FrmConsole_JS_VAR_removeGroupInviteLink =
    'window.WAPI.revokeGroupInviteLink("<#GROUP_ID#>");';
  FrmConsole_JS_VAR_checkNumberStatus =
    'window.WAPI.checkNumberStatus("<#PHONE#>");';
...
type
  TLanguageInject = (TL_Portugues_BR = 0, TL_English = 1, TL_Espanol = 2,
    TL_Farsi = 3);
  TConnectionDBType = (TCon_None = 0, TCon_Memory = 1, TCon_FireDAC = 2,
    TCon_DBExpress = 3, TCon_ADO = 4);

  TFormQrCodeType = (Ft_Desktop = 0, Ft_Http = 1, Ft_None = 2);
  // Form ou RestDataWare
  TSendFile_Image = (Tsf_Jpg = 0, Tsf_Jpeg = 1, Tsf_Tif = 2, Tsf_Ico = 3,
    Tsf_Bmp = 4, Tsf_Png = 5, Tsf_Raw = 6);

  TStatusType = (Inject_Initialized, Inject_Initializing, Inject_Destroying,
    Inject_Destroy, Server_Disconnected, Server_Disconnecting, Server_Connected,
    Server_ConnectedDown, Server_Connecting, Server_ConnectingNoPhone,
  ...
  TTypeHeader = (Th_None = 0,
    ...
    Th_GetAllGroupContacts = 4, Th_GetBatteryLevel = 5, Th_GetQrCodeForm = 6,
    Th_GetQrCodeWEB = 7, Th_GetMyNumber = 8, Th_OnChangeConnect = 9,
    Th_GetReserv1 = 10, Th_GetReserv2 = 11, Th_GetReserv3 = 12,
    Th_GetReserv4 = 13, Th_GetReserv5 = 14, Th_GetReserv6 = 15,
    Th_GetReserv7 = 16, Th_GetCheckIsValidNumber = 17,
    Th_GetCheckIsConnected = 18, Th_GetProfilePicThumb = 19,
    Th_getAllGroups = 20, Th_getAllGroupAdmins = 21,
    ...
    Th_GetStatusMessage = 38, Th_GetGroupInviteLink = 39, Th_GetMe = 40,
    Th_NewCheckIsValidNumber = 41);

Function VerificaCompatibilidadeVersao(PVersaoExterna: String;
  PversaoInterna: String): Boolean;
Function FrmConsole_JS_AlterVar(var PScript: String; PNomeVar: String;
...
Begin
  ...
  try
    PVersaoExterna := StringReplace(PVersaoExterna, ',', '.',
      [rfReplaceAll, rfIgnoreCase]);
    PversaoInterna := StringReplace(PversaoInterna, ',', '.',
      [rfReplaceAll, rfIgnoreCase]);
    ...
    for I := 0 to LVersionInt.count - 1 do
      LInt := LInt + Copy(LVersionInt.Strings[I] + '00000000', 0,
        Versao0porCasas);

    for I := 0 to LVersionExt.count - 1 do
      LExt := LExt + Copy(LVersionExt.Strings[I] + '00000000', 0,
        Versao0porCasas);
...
Begin
  ...
  if pos('>', PNomeVar) = 0 Then
    PNomeVar := PNomeVar + '>';
  PScript := StringReplace(PScript, PNomeVar, PValor,
    [rfReplaceAll, rfIgnoreCase]);
  Result := PScript;
end;
...
Begin
  ...
  try
    for I := 0 to 10 do
    begin
      Ltmp := LowerCase(Copy(GetEnumName(TypeInfo(TSendFile_Image),
        ord(TSendFile_Image(I))), 3, 50));
...
Begin
  ...
  Begin
    ...
    for I := 0 to LmaxCount do
    Begin
      LNome := LowerCase(GetEnumName(TypeInfo(TTypeHeader),
        ord(TTypeHeader(I))));
