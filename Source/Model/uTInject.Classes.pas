...
type
  ...
  TNotificationCenter = procedure(PTypeHeader: TTypeHeader; PValue: String;
    Const PReturnClass: TObject = nil) of object;
  TOnErroInternal = procedure(Sender: TObject; Const PError: String;
    Const PInfoAdc: String) of object;
  TDown_State = (TDw_Wait = 0, TDw_Start = 1, TDw_CanceledErro = 2,
    TDw_CanceledUser = 3, TDw_Completed = 4);
...
{$ENDIF}
  ...
  public
    ...
    constructor Create(pAJsonString: string;
      PJsonOption: TJsonOptions = JsonOptionClassPadrao);
    destructor Destroy; override;
  ...
  Public
    ...
    Property StatusDelivered: string Read FStatusDelivered
      Write FStatusDelivered;
  end;
  ...
  public
    ...
    property _inAutoreleasePool: Boolean Read F_inAutoreleasePool
      Write F_inAutoreleasePool;
  ...
  public
    ...
    property animationDuration: Extended Read FanimationDuration
      Write FanimationDuration;
    property animatedAsNewMsg: Boolean Read FanimatedAsNewMsg
      Write FanimatedAsNewMsg;
    property _swStreamingSupported: Boolean Read F_swStreamingSupported
      Write F_swStreamingSupported;
    property _listeningToSwSupport: Boolean Read F_listeningToSwSupport
      Write F_listeningToSwSupport;
  ...
  public
    ...
    property chatstates: TArray<TChatStatesClass> read FChatstates
      write FChatstates;
  ...
  public
    ...
    property participants: TArray<TParticipantsClass> read FParticipants
      write FParticipants;
    property pendingParticipants: TArray<TParticipantsClass>
      read FPendingParticipants Write FPendingParticipants; // @LuizAlvez
  end;
  ...
  private
    ...
    Fwa_version: String;
  public
    property device_manufacturer: String read Fdevice_manufacturer
      write Fdevice_manufacturer;
    ...
    property os_build_number: String read Fos_build_number
      write Fos_build_number;
  ...
  public
    ...
    property &type: String read Ftype write Ftype;
    property profilePicThumbObj: TProfilePicThumbObjClass
      read FProfilePicThumbObj write FProfilePicThumbObj;
  ...
  public
    ...
    property id: String read FId write FId;
    property selectedButtonId: String read FselectedButtonId
      write FselectedButtonId;
    property selectedDisplayText: String read FselectedDisplayText
      write FselectedDisplayText;
    ...
    property mediaKeyTimestamp: Extended read FmediaKeyTimestamp
      Write FmediaKeyTimestamp;
    ...
    property &type: String read Ftype write Ftype;
    property selectButtonID: String read FselectButtonID write FselectButtonID;
  end;
  ...
  public
    ...
    property isAnnounceGrpRestrict: Boolean read FIsAnnounceGrpRestrict
      write FIsAnnounceGrpRestrict;
    property groupMetadata: TGroupMetadataClass read FGroupMetadata
      write FGroupMetadata;
  ...
  private
    ...
    FTags: TQrCodeRets;
  public
    constructor Create(pAJsonString: string; PJsonOption: TJsonOptions = [];
      PTagRequired: TQrCodeRets = []);
    destructor Destroy; override;
...
Begin
  try
    ...
    Begin
      ...
      LName := GlobalCEFApp.LogConsole + 'ConsoleMessage' +
        FormatDateTime('yymmdd_HH', now) + '.log';

      if (not GlobalCEFApp.LogConsoleActive) or
        (GlobalCEFApp.LogConsole = '') Then
        Exit;

      if PCab = '' then
        LTmp := '[' + FormatDateTime('dd/mm/yy hh:nn:ss', now) + ']  '
      else if PCab = 'CONSOLE' then
        LTmp := '[' + FormatDateTime('dd/mm/yy hh:nn:ss', now) + ' - ' + PCab +
          ']  ' + slinebreak
      Else
        LTmp := '[' + FormatDateTime('dd/mm/yy hh:nn:ss', now) + ' - ' + PCab +
          ']  ' + slinebreak;
...
begin
  lAJsonObj := TJSONObject.ParseJSONValue(TEncoding.UTF8.GetBytes(pAJsonString),
    0); { TODO : mudei de ASCII para UTF8 aqui }
  try
...
begin
  ...
  if LowerCase(FKind) = LowerCase('chat') then
    FKindTypeNumber := TypContact
  else if LowerCase(FKind) = LowerCase('group') then
    FKindTypeNumber := TypGroup
  else
...
{ procedure TMessagesClass.SetselectButtonID(const Value: String);
  begin
  FselectButtonID := Value;
  end;

  TQrCodeClass }
constructor TQrCodeClass.Create(pAJsonString: string; PJsonOption: TJsonOptions;
  PTagRequired: TQrCodeRets);
var
  lCode: String;
  LAchou: Boolean;
...
{ TClassPadrao }
constructor TClassPadrao.Create(pAJsonString: string;
  ...
  try
    ...
    Except
      ...
      Begin
        ...
        Begin
          if Pos(AnsiUpperCase('Cold not load SSL'), AnsiUpperCase(E.Message)) > 0
          then
            Application.MessageBox(PWideChar(MSG_Exceptlibeay32dll),
              PWideChar(Application.Title), MB_ICONERROR + mb_ok)
          else
            Application.MessageBox
              (PWideChar('Erro HTTP GET (js.abr) ' + E.Message),
...
begin
  ...
  FNumbers.Text := StringReplace(FNumbers.Text, '{result:[', '',
    [rfReplaceAll]);
  FNumbers.Text := StringReplace(FNumbers.Text, ']}', '', [rfReplaceAll]);
end;
...
class function TClassAllGroupContacts.FromJsonString(AJsonString: string)
...
constructor TGetMeClass.Create(pAJsonString: string;
  PJsonOption: TJsonOptions = JsonOptionClassPadrao);
begin
