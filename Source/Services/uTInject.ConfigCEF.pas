...
type
  ...
  public
    ...
    property PathFrameworkDirPath: String Read FPathFrameworkDirPath
      Write SetPathFrameworkDirPath;
    property PathResourcesDirPath: String Read FPathResourcesDirPath
      Write SetPathResourcesDirPath;
    property PathLocalesDirPath: String Read FPathLocalesDirPath
      Write SetPathLocalesDirPath;
    property PathCache: String Read FPathCache Write SetPathCache;
    property PathUserDataPath: String Read FPathUserDataPath
      Write SetPathUserDataPath;
    ...
    property LogConsoleActive: Boolean Read FLogConsoleActive
      Write SetLogConsoleActive;
...
begin
  FPathJsUpdate := Now;
  UpdateIniFile('Tinject Comp', 'Ultima interação',
    FormatDateTime('dd/mm/yy hh:nn:ss', FPathJsUpdate));
end;
...
begin
  ...
  Begin
    FIniFIle.WriteString('Informacao', 'Aplicativo vinculado',
      Application.ExeName);
    ...
    LogConsoleActive := FIniFIle.ReadBool('Path Defines',
      'Log Console Active', false);
    PathFrameworkDirPath := FIniFIle.ReadString('Path Defines',
      'FrameWork', '');
    ...
    if LogConsole = '' then
      LogConsole := IncludeTrailingPathDelimiter
        (ExtractFilePath(Application.ExeName)) + 'LogTinject\';
  End;
...
begin
  ...
  if NomeNovoArquivo = '' then
  begin
    NomeNovoArquivo := NomeArquivoIni;
  end;
  FIniFIle := TIniFile.Create(FDirApp + NomeNovoArquivo);
  Lx := FIniFIle.ReadString('Tinject Comp', 'Ultima interação',
    '01/01/1500 05:00:00');
  ...
  Begin
    FErrorInt := true;
    LVReque := IntToStr(VersaoMinima_CF4_Major) + '.' +
      IntToStr(VersaoMinima_CF4_Minor) + '.' +
      IntToStr(VersaoMinima_CF4_Release);
    LVerIdent := IntToStr(CEF_SUPPORTED_VERSION_MAJOR) + '.' +
      IntToStr(CEF_SUPPORTED_VERSION_MINOR) + '.' +
      IntToStr(CEF_SUPPORTED_VERSION_BUILD);

    Application.MessageBox(PWideChar(Format(MSG_ConfigCEF_ExceptVersaoErrada,
      [LVReque, LVerIdent])), PWideChar(Application.Title),
      MB_ICONERROR + mb_ok);
    Result := false;
    Exit;
  End;
  ...
  UpdateIniFile('Tinject Comp', 'CEF4 Versão',
    IntToStr(CEF_SUPPORTED_VERSION_MAJOR) + '.' +
    IntToStr(CEF_SUPPORTED_VERSION_MINOR) + '.' +
    IntToStr(CEF_SUPPORTED_VERSION_RELEASE) + '.' +
    IntToStr(CEF_SUPPORTED_VERSION_BUILD));
  UpdateIniFile('Tinject Comp', 'CHROME Versão',
    IntToStr(CEF_CHROMEELF_VERSION_MAJOR) + '.' +
  ...
  try
    ...
    Result := (Self.status = asInitialized);
    if not Result then
      Application.MessageBox(PWideChar(MSG_ConfigCEF_ExceptConnection),
